...
/**
 * Ressource manager to manage the resources from overlap2D
 *
 */
public class PlatformerResourceManager extends ResourceManager {
  ...
  /**
   * Initialises all resources needed for this game
   */
  public void initPlatformerResources() {
    // Loading project configuration file that keeps list of available resolutions
    loadProjectVO();

    // figure out the best resolution from the list of available resolutions for the existing screen
    // size
    currentResolution = getBestResolutionMatch(getProjectVO());

    // Game stage width should be dynamic based on screen size, so if you have wider screen, instead
    // of stretching
    ...
    // Sets working resolution, so init all resources will load in to memory resources of that
    // resolution/size only
  ...
  /**
   * This method finds the closest match from the available texture resolutions generated by
   * overlap2d that are closest in size to the current screen size. The comparison goes by height.
   *
   * @param projctInfoVo
  ...
  public ResolutionEntryVO getBestResolutionMatch(ProjectInfoVO projctInfoVo) {
    ...
    for (int i = 0; i < projctInfoVo.resolutions.size(); i++) {
      float newDeltaSize =
          Math.abs(projctInfoVo.resolutions.get(i).height - Gdx.graphics.getHeight());
